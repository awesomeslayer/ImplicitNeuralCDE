...
class DataModule(LightningDataModule):
    def __init__(self, dsname, batch_size):
        ...
        Xval, Xtest, Yval, Ytest = train_test_split(
            Xvaltest, Yvaltest, test_size=0.6, random_state=42
        )
    ...
    def train_dataloader(self):
        return DataLoader(
            self.train_ds, batch_size=self.batch_size, num_workers=16, shuffle=True
        )

    def val_dataloader(self):
        return DataLoader(
            self.val_ds, batch_size=self.batch_size, num_workers=16, shuffle=False
        )

    def test_dataloader(self):
        return DataLoader(
            self.test_ds, batch_size=self.batch_size, num_workers=16, shuffle=False
        )
